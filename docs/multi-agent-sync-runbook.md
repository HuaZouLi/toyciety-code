...
Keep ToycietyCode's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between ToycietyCode settings and Codex runtime behavior
...
Run from `ToycietyCode` repo root:
...
## ToycietyCode Files To Update If Upstream Changes
...
- `src-tauri/src/bin/toyciety_code_daemon/rpc/codex.rs`
...
- Keep ToycietyCode scoped to the new key only (no legacy alias read/write).
- Keep ToycietyCode writes aligned with upstream expectations.
...
- Reconcile ToycietyCode guardrails when upstream changes.
...
- ToycietyCode default `agents.max_depth` is `1`.
- ToycietyCode currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
