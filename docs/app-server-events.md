...
- Which app-server events ToycietyCode supports right now.
- Which app-server requests ToycietyCode sends right now.
- Where to look in ToycietyCode to add support.
...
3. Compare Codex events vs ToycietyCode routing.
4. Compare Codex client request methods vs ToycietyCode outgoing request methods.
5. Compare Codex server request methods vs ToycietyCode inbound request handling.
6. Update supported and missing lists below.

Related project skill:
- `.codex/skills/app-server-events-sync/SKILL.md`

## Where To Look In ToycietyCode
...
- `src-tauri/src/bin/toyciety_code_daemon.rs`

## Supported Notifications (Codex v2)

These are the current Codex v2 `ServerNotification` methods that ToycietyCode
...
## Additional Stream Methods Handled In ToycietyCode
...
- `codex/backgroundThread` (ToycietyCode synthetic bridge event)
- `codex/connected` (ToycietyCode synthetic bridge event)
...
## Supported Requests (ToycietyCode -> App-Server, v2)

These are v2 request methods ToycietyCode currently sends to Codex app-server:
...
Compared against Codex v2 request methods, ToycietyCode currently does not send:
...
## Server Requests (App-Server -> ToycietyCode, v2)
...
3. List ToycietyCode routed methods:
...
4. List ToycietyCode outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - ToycietyCode generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - ToycietyCode attempts `turn/steer` only when steer capability is enabled, the thread is processing, and an active turn id exists.
  - If `turn/steer` fails, ToycietyCode does not fall back to `turn/start`; it clears stale processing/turn state when applicable, surfaces an error, and returns `steer_failed`.
