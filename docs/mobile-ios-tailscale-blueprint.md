# ToycietyCode iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted ToycietyCode backend over Tailscale.
...
1. Desktop ToycietyCode hosts the daemon and executes Codex workflows.
2. iOS ToycietyCode connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop ToycietyCode is installed and able to run local workspaces.
