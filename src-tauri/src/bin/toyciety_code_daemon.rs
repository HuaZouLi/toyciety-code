...
#[path = "../git_utils.rs"]
mod git_utils;
#[path = "toyciety_code_daemon/rpc.rs"]
mod rpc;
#[path = "../rules.rs"]
mod rules;
#[path = "../shared/mod.rs"]
...
#[path = "../storage.rs"]
mod storage;
#[path = "toyciety_code_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "toyciety-code-daemon";
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("toyciety-code-daemon");
        }
    }
    ...
    PathBuf::from(home)
        .join(".local")
        .join("share")
        .join("toyciety-code-daemon")
}

fn usage() -> String {
    format!(
        "\
USAGE:\n  toyciety-code-daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n\n\
...
mod tests {
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            codex_login_cancels: Mutex::new(HashMap::new()),
            daemon_binary_path: Some("/tmp/toyciety-code-daemon".to_string()),
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "toyciety-code-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
