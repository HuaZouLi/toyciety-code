...
pub(crate) fn daemon_binary_candidates() -> &'static [&'static str] {
    if cfg!(windows) {
        &["toyciety_code_daemon.exe", "toyciety-code-daemon.exe"]
    } else {
        &["toyciety_code_daemon", "toyciety-code-daemon"]
    }
}
...
mod tests {
    ...
    fn daemon_binary_candidates_prioritize_underscored_name() {
        assert!(daemon_binary_candidates()[0].starts_with("toyciety_code_daemon"));
    }
}
